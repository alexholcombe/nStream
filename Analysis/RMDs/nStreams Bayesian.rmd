...
BF~10~ is `r round(BF10, 2)`. There is strong evidence for a difference in mean latency between the two- and eight-streams conditions. Mean latency for the two-streams condition (M = `r latency %>% pull(twoStreams) %>% mean %>% round(2)`, SD = `r latency %>% pull(twoStreams) %>% sd %>% round(2)`) is less than the mean latency for the eight-streams condition (M = `r latency %>% pull(eightStreams) %>% mean %>% round(2)`, SD = `r latency %>% pull(eightStreams) %>% sd %>% round(2)`).
...
```
The Bayesian analysis for the efficacy data shows weak evidence for a lack of an effect (BF~10~ = `r round(BF10,2)`). The mean for the two-streams condition is `r efficacy %>% pull(twoStreams) %>% mean %>% round(2)` (SD = `r efficacy %>% pull(twoStreams) %>% sd %>% round(2)`). The mean for the eight-streams condition is `r efficacy %>% pull(eightStreams) %>% mean %>% round(2)` (SD = `r efficacy %>% pull(eightStreams) %>% sd %>% round(2)`).
```{r}
...
In this experiment, the program presented 8 streams on each trial. Prior to the onset of a trial, participants saw a circular array of hashmarks for 250ms [@goodbourn_pseudoextinction]. The possible position of the cue on that trial was indicated by white rings around either two or eight streams. These mimicked the position of the streams in the first experiment's conditions. This experiment tests whether any differences between the two or eight streams conditions are due to increasing interference that scales with the number of streams - this theory predicts no difference between conditions in this experiment - or due to a cost involved in spreading attention over a larger area of visual space - this theory predicts a replication of the initial experiment's effects. 
...
The latency for the two-precues condition (M = `r latency %>% pull(twoStreams) %>% mean %>% round(2)`, SD = `r latency %>% pull(twoStreams) %>% sd %>% round(2)`) is less than that of the eight-precues condition (M = `r latency %>% pull(eightStreams) %>% mean %>% round(2)`, SD = `r latency %>% pull(eightStreams) %>% sd %>% round(2)`; BF~10~ = `r round(BF10,2)`). 
...
precisionScatter <- ggplot(precision, aes(x=twoStreams, y=eightStreams, colour=ordered(1:nParticipants)))+
  geom_point(size = 4)+
  scale_color_brewer(palette='Spectral', name='Participant')+
  lims(x=c(40,100), y=c(40,100))+
  labs(title='Precision Estimates (ms)')+
...
There is weak evidence for no difference between the two conditions, BF~10~ = `r round(BF10,2)`. The two-precues condition has a mean of `r precision %>% pull(twoStreams) %>% mean %>% round(2)` (SD = `r precision %>% pull(twoStreams) %>% sd %>% round(2)`). The eight-precues condition has a mean of `r precision %>% pull(eightStreams) %>% mean %>% round(2)` (SD = `r precision %>% pull(eightStreams) %>% sd %>% round(2)`).
...
efficacyScatter <- ggplot(efficacy, aes(x=twoStreams, y=eightStreams, colour=ordered(1:nParticipants)))+
  geom_point(size = 4)+
  scale_color_brewer(palette='Spectral', name='Participant')+
  lims(x=c(0,1), y=c(0,1))+
  labs(title='Efficacy Estimates [1 - P(Guess)]', y = 'Eight Streams', y='Eight Streams')+
...
There is weak evidence for no difference between the two conditions, BF~10~ = `r round(BF10,2)`. The two-precues condition has a mean of `r efficacy %>% pull(twoStreams) %>% mean %>% round(2)` (SD = `r efficacy %>% pull(twoStreams) %>% sd %>% round(2)`). The eight-precues condition has a mean of `r efficacy %>% pull(eightStreams) %>% mean %>% round(2)` (SD = `r efficacy %>% pull(eightStreams) %>% sd %>% round(2)`).
