...
csl: Bibtex/apa.csl
---
How does the visual system select events in time? To answer this question, @goodbourn_pseudoextinction presented participants with 2 RSVP streams in which either one or two simultaneous items were cued for a report. Each response participants gave was mapped to a particular point in time relative to the cued item on the relevant trial and the resulting distribution of serial position errors (SPEs) were analysed. Before discussing their results it is worth considering the sort of distribution we might expect if items are selected using at attention shift to the cue. The cue was a white circle with a rapid onset, exactly the sort of stimulus we would expect to elicit an exogenous attention shift [@carrasco_visual_2011; @muller_reflexive_1989; @nakayama_sustained_1989]. All RSVP distributions will contain trials where participants guessed because they did not detect the cue. These trials will contribute a uniform component to the distribution. The other component of the distribution will be responses informed by the cue. These do not have to be accurate responses. Instead, they can reflect the variance of a temporal binding process, the items selected after an attention shift, or some other process. @goodbourn_pseudoextinction fit their data with a mixture of a uniform and a Gaussian (Zhang & Luck??). They found that the nonguessing component included items from at the cue or before. This is unlikely (impossible?) under an attention shift account, where selection is triggered by the cue. @goodbourn_pseudoextinction explained their results by positing a buffer of visual information that rapidly decays. One item from this buffer is selected for tokenisation and subsequently consolidation into working memory based on its coincidence with the cue. The process that determines coincidence is error-prone, and this causes participants to percieve items that occured before or after the cue as occuring with the ring. 

The first two of these impart particular non-guessing distributions to the SPE distribution. If participants select items from the cued stream using an attention shift, the SPE distribution will have a positively skewed non-guessing component. An attention shift is an entirely post-stimulus effect. It will not trigger until the cue is detected. Thus, we expect the non-guessing component to not include any responses from at the cue or before. 
