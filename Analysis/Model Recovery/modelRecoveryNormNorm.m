            ...
            for precision = precisions
                ...
                simulatedData(thisRow,:) = theseErrors;
                    ...
                    for thisReplicate = 1:nReplicates
                        ...
                        parameterLowerBound = [0 -0.1 10^-8]; %might have to replace the mu bound with small nonzero number. Not clear if bounds are open or not. Sigma cannot be 1 or less
                    ...
                    if h==0
                        text(max(theseErrors)-4, max(heights)*.8, 'Efficacy = 0')
                        line(grid, pdf_uniformonly(grid,1))
                    else
                        text(max(theseErrors)-4, max(heights)*.8, strcat('Efficacy = ',num2str(bestEstimatesCombined(1))));
                        text(max(theseErrors)-4, max(heights)*.7, strcat('Latency = ',num2str(bestEstimatesCombined(2))));
                        text(max(theseErrors)-4, max(heights)*.6, strcat('Precision = ',num2str(bestEstimatesCombined(3))));
