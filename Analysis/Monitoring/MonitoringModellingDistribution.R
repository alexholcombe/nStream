...
##This program simulates the SPE distribution, rather than the process ####################
###########################################################################################
rm(list=ls())
library(truncnorm)
library(dplyr)
    ...
    for(thisTrial in 1:nTrials){
      ...
      if(nonGuess){
        if(thisCuedStream %in% theseMonitoredStreams){
            thisSPE <- rnorm(1, mean = .39, sd = 0.87) #Latency and Precision from fit to single stream condition of G&H exp 1.
            thisSPE <- round(thisSPE)
            thisSP <- thisSPE + thisCueTemporalPos
            while(thisSPE < minSPE | thisSPE > maxSPE){
              thisSPE <- rnorm(1, mean = .39, sd = 0.87) #Latency and Precision from fit to single stream condition of G&H exp 1.
              thisSPE <- round(thisSPE)
            }
          ...
          thisSPE <- rnorm(1, mean = 1.2, sd = .87)
          thisSPE <- round(thisSPE)
