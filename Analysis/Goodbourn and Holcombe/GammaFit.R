...
densities <- paramsDF %>% #Purrr is new to me
  select(participant,latency, precision, model)%>% #Select the columns with the variables we want
  pmap_dfr(function(latency, precision, participant, model){ #For each row, compute the density over a range of milliseconds and return a dataframe
    ...
    data.frame(                                       #pmap_dfr iterates over multiple arguments (cells in each row) simultaneously
      ID = participant,                      #It expects a df from the function and binds the output together by row, hence _dfr (dataframe row). Perfect for ggplot
      SPE = SPE,
      density = density,
      model = model,
      stringsAsFactors = F
...
for(thisID in IDs){
  ...
  ggsave(filename = paste0('GammaPlots/',thisID,'.png'),
