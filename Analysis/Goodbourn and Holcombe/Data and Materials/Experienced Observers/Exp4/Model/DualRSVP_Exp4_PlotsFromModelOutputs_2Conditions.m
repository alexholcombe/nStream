...
cd('/Users/experimentalmode/Documents/MATLAB/DualRSVP/Exp4/Model/');

%addpath(genpath('/Users/ptg/Dropbox/MATLAB/'));
%cd('/Users/ptg/Dropbox/MATLAB/DualRSVP/Exp4/Model/');
...
modelParamIcon_S1 = 'ko';
modelParamIcon_S2 = 'kv';

% Load data files

load('Outputs_for5_2Conditions.mat');
...
modelParameterAxes = [0.5 2.5 -0.1 1.1;
                      0.5 2.5 -1.2 1.2;
                      0.5 2.5 -0.2 2.2];
                  
modelParameterAxes_byTime = [0.5 2.5 -0.1 1.1;
                             0.5 2.5 -180 180;
                             0.5 2.5 -30 330];
...
for thisParameter = 1:nFreeParameters
    ...
    for thisStream = 1:nStreams
        ...
        scatter(1:2,allEstimates_Combined(:,thisStream,thisParameter),modelParamIcon);
        
        box on;
        axis(modelParameterAxes(thisParameter,:));
        axis square;
        set(gca,'TickDir','out','TickLength', [0.025 0.05],'XMinorTick','off','YMinorTick','on','XTick',1:2,'XTickLabel',conditionLabels);
    end
...
for thisParameter = 1:nFreeParameters
    ...
    for thisStream = 1:nStreams
        ...
        scatter(1:2,theseMeans,modelParamIcon);
...
for thisParticipant = 1:nParticipants
    ...
    for thisParameter = 1:nFreeParameters
        ...
        for thisStream = 1:nStreams
            ...
            scatter(1:2,allEstimates_byParticipant(:,thisStream,thisParticipant,thisParameter),modelParamIcon);
...
for thisParameter = 1:nFreeParameters
    ...
    for thisStream = 1:nStreams
        ...
        for thisCondition = 1:nConditions
            ...
            line([thisCondition thisCondition],[theseLowerBounds theseUpperBounds], 'Color', [0 0 0]);
            hold on;
        ...
        scatter(1:2,theseEstimates,modelParamIcon);
...
set(gca,'YDir','normal','TickDir','out','TickLength', [0.025 0.05], ...
    ...
    % Plot contingent model parameters as a function of presentation rate

% Axis Labels:  x, 'Item Rate'; y, 'Model Parameter';
% Row Labels: (Participants) 'Combined', 'Mean', 'AH', 'SM', 'EN', 'PG';
% Column Labels: (Parameter) '(1-Suppression)', 'Delay (ms)', 'Diffusion (ms)';

contingentModelParameters_byTime = figure('color','white','name', 'Contingent Model Parameters (By Time)');
thisPlot = 0;
    ...
    for thisParameter = 1:nFreeParameters
        ...
        for thisStream = 1:nStreams
            ...
            if thisStream==1
                modelParamIcon = contingentModelIcon_S1;
            else
                modelParamIcon = contingentModelIcon_S2;
            end

            for thisCondition = 1:nConditions

                standardLowerBounds = allLowerBounds_Combined(thisCondition,thisStream,thisParameter);
                standardUpperBounds = allUpperBounds_Combined(thisCondition,thisStream,thisParameter);
                
                targetContingentLowerBounds = allLowerBounds_Contingent_Combined(thisCondition,thisStream,1,thisParameter);
                targetContingentUpperBounds = allUpperBounds_Contingent_Combined(thisCondition,thisStream,1,thisParameter);
                
                errorContingentLowerBounds = allLowerBounds_Contingent_Combined(thisCondition,thisStream,2,thisParameter);
                errorContingentUpperBounds = allUpperBounds_Contingent_Combined(thisCondition,thisStream,2,thisParameter);

                if thisParameter~=1

                    standardLowerBounds = standardLowerBounds*itemDurations(thisCondition);
                    standardUpperBounds = standardUpperBounds*itemDurations(thisCondition);
                    
                    targetContingentLowerBounds = targetContingentLowerBounds*itemDurations(thisCondition);
                    targetContingentUpperBounds = targetContingentUpperBounds*itemDurations(thisCondition);
                    
                    errorContingentLowerBounds = errorContingentLowerBounds*itemDurations(thisCondition);
                    errorContingentUpperBounds = errorContingentUpperBounds*itemDurations(thisCondition);

                end

                line([thisCondition thisCondition]+contingentModelJitter(1),[standardLowerBounds standardUpperBounds], 'Color', [0 0 0]);
                line([thisCondition thisCondition]+contingentModelJitter(2),[targetContingentLowerBounds targetContingentUpperBounds], 'Color', contingentTargetColor);
                line([thisCondition thisCondition]+contingentModelJitter(3),[errorContingentLowerBounds errorContingentUpperBounds], 'Color', contingentErrorColor);

                hold on;

            end

            standardEstimates = allEstimates_Combined(:,thisStream,thisParameter);
            targetContingentEstimates = allEstimates_Contingent_Combined(:,thisStream,1,thisParameter);
            errorContingentEstimates = allEstimates_Contingent_Combined(:,thisStream,2,thisParameter);

            if thisParameter~=1
                standardEstimates = standardEstimates'.*itemDurations;
                targetContingentEstimates = targetContingentEstimates'.*itemDurations;
                errorContingentEstimates = errorContingentEstimates'.*itemDurations;
            end

            scatter((1:2)+contingentModelJitter(1),standardEstimates,modelParamIcon{1});
            scatter((1:2)+contingentModelJitter(2),targetContingentEstimates,modelParamIcon{2});
            scatter((1:2)+contingentModelJitter(3),errorContingentEstimates,modelParamIcon{3});
    ...
    subplot(nParticipants+2,nFreeParameters,thisPlot);
        ...
        for thisStream = 1:nStreams
            ...
            standardEstimates = allEstimates_byParticipant(:,thisStream,:,thisParameter);
            targetContingentEstimates = allEstimates_Contingent_byParticipant(:,thisStream,:,1,thisParameter);
            errorContingentEstimates = allEstimates_Contingent_byParticipant(:,thisStream,:,2,thisParameter);
            
            standardMeans = nanmean(standardEstimates,3);
            targetContingentMeans = nanmean(targetContingentEstimates,3);
            errorContingentMeans = nanmean(errorContingentEstimates,3);
            
            standardSEMs = nanstd(standardEstimates,[],3)./sqrt(sum(~isnan(standardEstimates),3));
            targetContingentSEMs = nanstd(targetContingentEstimates,[],3)./sqrt(sum(~isnan(targetContingentEstimates),3));
            errorContingentSEMs = nanstd(errorContingentEstimates,[],3)./sqrt(sum(~isnan(errorContingentEstimates),3));
            
            if thisParameter~=1
                
                standardMeans = standardMeans'.*itemDurations;
                targetContingentMeans = targetContingentMeans'.*itemDurations;
                errorContingentMeans = errorContingentMeans'.*itemDurations;
                
                standardSEMs = standardSEMs'.*itemDurations;
                targetContingentSEMs = targetContingentSEMs'.*itemDurations;
                errorContingentSEMs = errorContingentSEMs'.*itemDurations;
                
            end

            standardLowerBounds = standardMeans-standardSEMs;
            standardUpperBounds = standardMeans+standardSEMs;
                
            targetContingentLowerBounds = targetContingentMeans-targetContingentSEMs;
            targetContingentUpperBounds = targetContingentMeans+targetContingentSEMs;
            
            errorContingentLowerBounds = errorContingentMeans-errorContingentSEMs;
            errorContingentUpperBounds = errorContingentMeans+errorContingentSEMs;
            
            
            for thisCondition = 1:nConditions
                    
                    
                line([thisCondition thisCondition]+contingentModelJitter(1),[standardLowerBounds(thisCondition) standardUpperBounds(thisCondition)], 'Color', [0 0 0]);
                line([thisCondition thisCondition]+contingentModelJitter(2),[targetContingentLowerBounds(thisCondition) targetContingentUpperBounds(thisCondition)], 'Color', contingentTargetColor);
                line([thisCondition thisCondition]+contingentModelJitter(3),[errorContingentLowerBounds(thisCondition) errorContingentUpperBounds(thisCondition)], 'Color', contingentErrorColor);
            ...
            scatter((1:2)+contingentModelJitter(1),standardMeans,modelParamIcon{1});
            scatter((1:2)+contingentModelJitter(2),targetContingentMeans,modelParamIcon{2});
            scatter((1:2)+contingentModelJitter(3),errorContingentMeans,modelParamIcon{3});
        ...
        for thisParameter = 1:nFreeParameters
            ...
            for thisStream = 1:nStreams
                ...
                for thisCondition = 1:nConditions
                    ...
                    standardLowerBounds = allLowerBounds_byParticipant(thisCondition,thisStream,thisParticipant,thisParameter);
                    standardUpperBounds = allUpperBounds_byParticipant(thisCondition,thisStream,thisParticipant,thisParameter);

                    targetContingentLowerBounds = allLowerBounds_Contingent_byParticipant(thisCondition,thisStream,thisParticipant,1,thisParameter);
                    targetContingentUpperBounds = allUpperBounds_Contingent_byParticipant(thisCondition,thisStream,thisParticipant,1,thisParameter);

                    errorContingentLowerBounds = allLowerBounds_Contingent_byParticipant(thisCondition,thisStream,thisParticipant,2,thisParameter);
                    errorContingentUpperBounds = allUpperBounds_Contingent_byParticipant(thisCondition,thisStream,thisParticipant,2,thisParameter);
                ...
                standardEstimates = allEstimates_byParticipant(:,thisStream,thisParticipant,thisParameter);
                targetContingentEstimates = allEstimates_Contingent_byParticipant(:,thisStream,thisParticipant,1,thisParameter);
                errorContingentEstimates = allEstimates_Contingent_byParticipant(:,thisStream,thisParticipant,2,thisParameter);
