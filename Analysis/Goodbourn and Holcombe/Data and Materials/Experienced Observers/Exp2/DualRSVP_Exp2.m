...
userDataFile = [participantID '_DualRSVP_Exp2_UserData.mat'];
...
if ~exist(userDataFile,'file')
   ...
   % Make balanced latin square
   bls_V = 1:nConditions;
   bls_CSI = [bls_V ; -bls_V];
   bls_M = zeros(nConditions);
   bls_M(:,1) = bls_V(:);
   
   for thisCol = 2:nConditions
       bls_M(:,thisCol) = rem(bls_V(:)+bls_CSI(thisCol-1)+(nConditions-1),nConditions) + 1;
   end
  
   % Randomise
   bls_R = randperm(nConditions);
   blockOrder = bls_R(bls_M);
...
ppm = 2.70;             % Pixels per mm (GDM, measured 18/7/12)
...
letterArray = [char(65:77) char(79:85) char(87:90) char(32)];      % A to Z plus space
nLetters = length(letterArray)-1;   % Number of possible letters
letterFont = 'Menlo';          % Presentation font - currently MUST be fixed width for drawing accuracy
letterTest = 'O';               % Letter for testing letter subtense
letterNull = '-';               % Placeholder letter for response screen
letterSize = 4;                 % Height of letter in degrees of visual angle
letterPoints = 80;              % Startng font size in points (not important, adjusted later)
letterStyle = 1;                % 0 normal, 1 bold
letterSeparation = 6;           % Distance between letters in degrees of visual angle
...
cueSubtense = 4.5;              % Subtense of cue in degrees
...
okPoints = 16;                  % Font size for 'OK' on response screen
...
instructionText2 = ['At some time during the trial, a white circle will appear ' ...
    ...
    'except for N and V, which will never appear.\n\n' ...
    'Click the mouse button to continue.'];

instructionText3 = ['There will be four conditions, each slightly different. ' ...
    'These differences will be explained during the practice trials.\n\n' ...
    'Click the mouse button to continue.'];

%     '(a) There are two streams, with a target letter in each. ' ...
%     'The target letters will appear simultaneously. Your task '...
%     'is to report both letters, and indicate in which stream they appeared.\n\n' ...
%     '(b) There are two streams, with a target letter in only one stream. ' ...
%     'Your task is to report the letter, and indicate in which stream it appeared.\n\n'...
%     '(c) There are two streams, with a target letter in each stream. ' ...
%     'However, a cue will appear before the trial to indicate the stream from which ' ...
%     'you need to report a target (the other stream can be ignored). Your task ' ...
%     'is to report the letter in the pre-cued stream.\n\n' ...
%     '(d) There is one stream, which may appear on the left or right side.' ...
%     'The side will be cued before the trial. Your task is to report the ' ...
%     'target letter.'];

instructionText4 = ['Please keep your eyes fixated on the white dot at ' ...
    'the centre of the screen at all times during the trial. ' ...
    'You should do this even when you only need to monitor one stream.\n\n' ...
...
if ~skipInstructions
    ...
    for thisTrial = 1:nPracticeTrials
        ...
        elseif thisCondition==3
            % Two streams, two targets, one pre-cued
            thisStream = randi(2);
            preCue = 1;
        elseif thisCondition==4
            % One stream, one target, pre-cued
            thisStream = randi(2);
            letterStream(3-thisStream,:) = (nLetters+1)*ones(1,nItems);
            thisTarget(3-thisStream) = NaN;
            preCue = 1;
        end
            ...
            if ceil((thisTrial+1)/(nPracticeTrials/nConditions))>ceil(thisTrial/(nPracticeTrials/nConditions))
                ...
                if nextSet==2
                    ...
                    nextTrialInfoText = ['In this second set of practice trials, ' ...
                        ...
                        'will be only one target letter. This may appear in either stream. ' ...
                ...
                elseif nextSet==3
                    % Condition 3
                    nextTrialInfoText = ['In this third set of practice trials, ' ...
                        'there will again be two streams. This time, one of the '...
                        'streams will be pre-cued before any letters have appeared. ' ...
                        'During the trial, a circled letter will appear in both streams, ' ... 
                        'but you only need to take note of the target letter in ' ...
                        'the pre-cued stream. Your task is to report the target letter ' ...
                        'in the pre-cued stream.\n\n' ...
                        'Click the mouse button to start the next set of practice trials.\n\n' ...
                        '(Trial ' num2str(thisTrial+1) ' of ' num2str(nPracticeTrials) ')'];
                elseif nextSet==4
                    % Condition 4
                    nextTrialInfoText = ['In this final set of practice trials, ' ...
                        'there will be only one stream, which may appear on either '...
                        'the left or the right of the screen. The side will be pre-cued ' ...
                        'before any letters have appeared. Your task is to report ' ...
                        'the target letter.\n\n' ...
                        'Click the mouse button to start the next set of practice trials.\n\n' ...
                        '(Trial ' num2str(thisTrial+1) ' of ' num2str(nPracticeTrials) ')'];
                end
...
for thisBlock = 1:nConditions
    ...
    preCue = (conditionNum==3)||(conditionNum==4);
    ...
    elseif conditionNum==3
        blockInstructionText = ['In this block, ' ...
        'there will be two streams. One of the '...
        'streams will be pre-cued before any letters have appeared. ' ...
        'During the trial, a circled letter will appear in both streams. ' ... 
        'Your task is to report the target letter in the pre-cued stream.\n\n' ...
        'Click the mouse button to begin the first trial.'];
    ...
    if thisBlock < nConditions
                
        remainingBlocks = nConditions-thisBlock;
