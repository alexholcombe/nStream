...
cd('/Users/experimentalmode/Documents/MATLAB/DualRSVP_SONA/Exp3/Model');
load('DualRSVP_SONA_Exp3_ModelOutputs.mat');

% Cull participants
% theseParticipants = setdiff(1:20,[3 16 19 20]);
theseParticipants = setdiff(1:20,[9 14]);

allEstimates_byParticipant_Naive = allEstimates_byParticipant(:,:,theseParticipants,:);
compiledErrors_Naive = compiledErrors(:,theseParticipants,:,:,:);
EHI_Naive = EHI(theseParticipants);
allAge_Naive = allAge(theseParticipants);
allSex_Naive = allSex(theseParticipants);

cd('/Users/experimentalmode/Documents/MATLAB/DualRSVP/Exp4/Model');
load('DualRSVP_Exp4_Outputs_2Conditions.mat');

theseParticipants = 1:6;

allEstimates_byParticipant_Exp = allEstimates_byParticipant(:,:,theseParticipants,:);
compiledErrors_Exp = compiledErrors(:,theseParticipants,:,:,:);
EHI_Exp = [325 325 -275 200 375 300];
allAge_Exp = NaN(1,6);
allSex_Exp = [1 0 0 1 1 1];

% Combine
allEstimates_byParticipant = cat(3,allEstimates_byParticipant_Naive,allEstimates_byParticipant_Exp);
compiledErrors = cat(2,compiledErrors_Naive,compiledErrors_Exp(:,:,1:2,:,:),compiledErrors_Exp(:,:,3:4,:,:));
EHI = [EHI_Naive EHI_Exp];
allAge = [allAge_Naive allAge_Exp];
allSex = [allSex_Naive allSex_Exp];


% Experiment 3 has 2 model conditions
allEstimates_byParticipant = allEstimates_byParticipant([2 1],:,:,:);
compiledErrors = compiledErrors([2 1],:,:,:,:);

allEstimates_byParticipant(2,:,:,:) = allEstimates_byParticipant(2,[2 1],:,:);
compiledErrors(2,:,:,:,:) = compiledErrors(2,:,:,:,[2 1]);
