...
cd('/Users/experimentalmode/Documents/MATLAB/DualRSVP_SONA/Exp1/Model');
load('DualRSVP_SONA_Exp1_ModelOutputs.mat');

% Cull participants
theseParticipants = setdiff(1:20,19);

allEstimates_byParticipant = allEstimates_byParticipant(:,:,theseParticipants,:);
allEstimates_byParticipant_Naive = allEstimates_byParticipant([1 4],:,:,:);
allEstimates_byParticipant_Naive = cat(1,allEstimates_byParticipant_Naive,NaN(size(allEstimates_byParticipant_Naive)));
compiledErrors_Naive = compiledErrors(1,theseParticipants,:,:,:);
...
load('DualRSVP_Exp2_Outputs_Final.mat');

theseParticipants = setdiff(1:6,3);

allEstimates_byParticipant_Exp = allEstimates_byParticipant(:,:,theseParticipants,:);
compiledErrors_Exp = compiledErrors(1,theseParticipants,:,:,:);
EHI_Exp = [400 375 350 325 400 400];
EHI_Exp = EHI_Exp(theseParticipants);
allAge_Exp = NaN(1,6);
allSex_Exp = [0 1 0 1 1 1];

% Combine
allEstimates_byParticipant = cat(3,allEstimates_byParticipant_Naive,allEstimates_byParticipant_Exp);
compiledErrors = cat(2,compiledErrors_Naive,compiledErrors_Exp);
EHI = [EHI_Naive EHI_Exp];
allAge = [allAge_Naive allAge_Exp];
allSex = [allSex_Naive allSex_Exp];

nParticipants = size(allEstimates_byParticipant,3);
nParameters = 3;
nConditions = 4;
nPositions = 2;
