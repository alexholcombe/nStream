...
nParamFiles <- length(list.files(path = 'Gamma Fits',pattern ='parameterEstimates.*\\.csv',full.names = T)) #How many parameter DFs are saved?

if(nParamFiles>0){
  print('we out here')
  paramFiles <- list.files(path = 'Gamma Fits',pattern ='parameterEstimates.*\\.csv',full.names = T) #What are the saved param files?
  ...
  for(thisParticipant in notModelled){
    for(thisCondition in unique(allErrors$crowded)){
      for(thisRing in unique(allErrors$ring)){
        for(thisModel in c("Normal", "Gamma")){
          cat('Participant: ', thisParticipant, '. Ring: ', thisRing, '. Condition: ', thisCondition, '                                         \r')
          
          try({
            invisible( #invisible combined with capture output hides those print 'error' statements hidden deep in mixRSVP::
              capture.output(
                theseParams <- allErrors %>% 
                  filter(., crowded == thisCondition, ring == thisRing, ID == thisParticipant) %>% 
                  analyzeOneCondition(., 24, parameterBounds(thisModel), nReps, thisModel)
              )
            )
          ...
          if(theseParams$pLRtest<.05){
            params %<>%
              mutate(efficacy=replace(efficacy, ID == thisParticipant & crowded == thisCondition & ring == thisRing & model == thisModel, theseParams$efficacy)) %>%
              as.data.frame()
            
            params %<>%
              mutate(latency=replace(latency, ID == thisParticipant & crowded == thisCondition & ring == thisRing & model == thisModel, theseParams$latency)) %>%
              as.data.frame()
            
            params %<>%
              mutate(precision=replace(precision, ID == thisParticipant & crowded == thisCondition & ring == thisRing & model == thisModel, theseParams$precision)) %>%
              as.data.frame()
          } else {
            params %<>%
              mutate(efficacy=replace(efficacy, ID == thisParticipant & crowded == thisCondition & ring == thisRing & model == thisModel, 0)) %>%
              as.data.frame()
            
            params %<>%
              mutate(latency=replace(latency, ID == thisParticipant & crowded == thisCondition & ring == thisRing & model == thisModel, NaN)) %>%
              as.data.frame()
            
            params %<>%
              mutate(precision=replace(precision, ID == thisParticipant & crowded == thisCondition & ring == thisRing & model == thisModel, NaN)) %>%
              as.data.frame()
          }
          
          params %<>%
            mutate(val=replace(val, ID == thisParticipant & crowded == thisCondition & ring == thisRing & model == thisModel, theseParams$val)) %>%
            as.data.frame()
          
          params %<>%
            mutate(valGuessing=replace(valGuessing, ID == thisParticipant & crowded == thisCondition & ring == thisRing & model == thisModel, theseParams$valGuessing)) %>%
            as.data.frame()
          
          params %<>%
            mutate(pLRtest=replace(pLRtest, ID == thisParticipant & crowded == thisCondition & ring == thisRing & model == thisModel, theseParams$pLRtest)) %>%
            as.data.frame()
        }
      }
    }
  }
  write.csv(params, paste0('Gamma Fits/parameterEstimates',format(Sys.time(), "%d-%m-%Y_%H-%M-%S"),'.csv'),row.names = F)
}
...
bounds <- parameterBounds(modelKind = 'Normal')
...
efficacyBF <- generalTestBF(efficacy ~ eccentricity * crowded + ID, 
...
efficacyInclusionBFs <- data.frame(
  variable = c('crowdedCoded','eccentricity','eccentricity:crowdedCoded'),
  BF = -999,
...
efficacyPosterior <- posterior(efficacyBF[9], iterations = 10000,progress = FALSE)
...
```{r}
latencyPosterior <- posterior(latencyBF[9], iterations = 10000, progress = FALSE)
...
```{r echo = FALSE}
precisionPosterior <- posterior(precisionBF[9], iterations = 10000, progress = FALSE)
