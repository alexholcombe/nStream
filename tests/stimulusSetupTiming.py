...
for nStreams in nStreamsPossibilities:
    for task in [ tasks[4] ]:  #T1 task is just for the single-target tasks, but both streams are presented
       ...
       for targetLeftRightIfOne in  ['right']: # ['left','right']: #If single target, should it be on the left or the right?
        anglesMustBeMultipleOf = 180 #int( round(360 / max(nStreamsPossibilities) ) )
...
nCol = nReps * len(possibleCueTemporalPositions) * len(range(0,360,anglesMustBeMultipleOf))
...
nStreamCounts = np.full(shape = len(nStreamsPossibilities), 0, int)
...
while n < trials.nTotal:
	...
	for repetition in range(nReps):
		...
		row = nStreamsPossibilities.index(trial['nStreams'])
		col = nStreamCounts[row]

		timing[row,col] = end - start
