...
if cueType == 'endogenous':
    cueColor = [1,-1,-1]
letterColor = [1.,1.,1.]
cueRadius =  1 #Charlie 2.5 #6 deg, as in Martini E2    Letters should have height of 2.5 deg
...
numRings = 1 # 3
streamsPerRingPossibilities = np.array([6])  #  2, 8]) #this needs to be listed here so when print header can work out the maximum value
...
trialsPerConditionDualStream = 1
...
ltrHeight =  8  #In charlie experiment: 3 #Martini letters were 2.5deg high
cueOffsets =  [5] #Charlie used [3,7,11.5]
...
#                posThis = [newX,newY]
            ...
            cueRadiusThis =  .75*calcLtrHeightSize( ltrHeight, cueOffsets, ringNum=int(streamI/streamsPerRing) ) #Charlie used 1.05*calcLtrHeightSize( ltrHeight, cueOffsets, ringNum=int(streamI/streamsPerRing) )
