...
allErrors <- expand.grid(experimentPhase = numeric(1),
  trialnum = 1:180,
  subject = character(6),
  task = character(1),
  noisePercent = numeric(1),
  targetLeftRightIfOne = character(1),
  resp0 = character(1),
  answer0 = character(1),
  correct0 = numeric(1),
  whichStream0 = numeric(1),
  whichRespCue0 = numeric(1),
  responsePosRelative0 = numeric(1),
  streamLtrSequence0 = character(1),
  streamLtrSequence1 = character(1),
  streamLtrSequence2 = character(1),
  streamLtrSequence3 = character(1),
  streamLtrSequence4 = character(1),
  streamLtrSequence5 = character(1),
  timingBlips = numeric(1),
  correctPos = numeric(1),
  responsePos = numeric(1), 
  condition = c('Exogenous','Endogenous'),
...
for(group in names(dataSets)){
	for(participant in names(dataSets[[group]])){
	    for(dateString in names(dataSets[[group]][[participant]])){
	      allErrorRowEnd <- allErrorRowStart+179
	      ...
	      tempKludged$condition <- ifelse(group == 'End6Strm82msSOA', 'Endogenous', 'Exogenous')
	      
	      allErrors[allErrorRowStart:allErrorRowEnd,] <- tempKludged
	      allErrorRowStart <- allErrorRowEnd+1
...
write.csv(allErrors,'Analysis/allErrors.csv')
