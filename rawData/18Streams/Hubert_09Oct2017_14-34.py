...
nStreamsPossibilities = [2] #np.arange(2,21,3) #this needs to be listed here so when print header can work out the maximum value
rings = [2] #range(int(np.ceil(float(max(nStreamsPossibilities))/streamsPerRing)))
cues = [True, False]
...
def doRSVPStim(trial):
    ...
    for thisFrame in xrange(numLettersToPresent):
        ...
        hashes = list()
        hashes.append(fixatn)

        hashesCounterPhase = list()
        hashesCounterPhase.append(fixatnCounterphase)
        ...
        for thisStream in xrange(nStreams):
            ...
            thisHash = corticalMagnification.corticalMagnification( #Scale the height (and thus the width for a monospaced font like Sloan) based on cortical magnification estimate. This function returns the stimulus, not its size
            stimulus = thisHash,
            ltrHeight = ltrHeight,
            cue = False)

            hashes.append(thisHash)
            hashesCounterPhase.append(thisHash)
        ...
        thisHashStimuli = visual.ElementArrayStim( #A stimulus representing this frame with the fixation at full luminance
            win = myWin,
            units = 'pix',
            nElements=1,
            xys = [[0,0]],
        ...
        thisHashStimuliCounterPhase = visual.ElementArrayStim( #A stimulus representing this frame with the fixation phase reversed
            win = myWin,
            units = 'pix',
            nElements=1,
            xys = [[0,0]],
        ...
        hashStimuli.append([thisHashStimuli, thisHashStimuliCounterPhase])
