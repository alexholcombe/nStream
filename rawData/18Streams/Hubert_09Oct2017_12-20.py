...
rings = [0,1,2] #range(int(np.ceil(float(max(nStreamsPossibilities))/streamsPerRing)))
...
def doRSVPStim(trial):
    ...
    if  (trials.nTotal/3) % nDone == 0 or nDone == trials.nTotal/2:
        startTrialStimuli.text = startTrialStimuli.text + 'This is trial ' + str(nDone) + ' of ' + str(trials.nTotal)    
    else:
        startTrialStimuli.text = 'Click here to start the trial'
