...
modelNames = {'truncNormalXMinEstimate','normal'};
...
for thisModel = 1:nModels
    ...
    for thisSample = 1:nSamples
        ...
        [thisAICCombined thisBICCombined] = aicbic(-allMinNegLogLikelihoods_Combined(thisSample),nParams(thisModel), allNTrials_Combined(thisSample));
    ...
    for thisSample = 1:nSamples
       ...
       for thisParticipant = 1:thisNParticipants
           [thisAICByParticipant thisBICByParticipant] = aicbic(-allMinNegLogLikelihoods_byParticipant(thisSample, thisParticipant),nParams(thisModel), allNTrials_byParticipant(thisSample, thisParticipant))
...
csvFile = fopen('../BFCombinedXMinEstimate.csv','w');
fprintf(csvFile, 'twoStreams, eightStreams, End6Strm82msSOA,  Ex6Strm82msSOA \n');
dlmwrite('../BFCombinedXMinEstimate.csv', BFCombined', '-append', 'precision','%i');

writeFile = fopen('../BF_ByParticipantXMinEstimate.csv','w');  % Overwrite file
