...
streamGroups = {'SONA/Confidence/twoStreams/','SONA/Confidence/eightStreams/'};
confidenceGroups = {'High', 'Low'}
...
dataFormat = '%s%d%s%s%d%s%d%d%s%d%d%s%d%d%d%d%s%s%s%s%s%s%s%s%d%d%s'
...
nStreamGroups = numel(streamGroups);
nConfidenceGroups = numel(confidenceGroups)
allParticipantsGlobal = repmat({''},nStreamGroups,nConfidenceGroups, 10);

% Cycle through each group
for thisStreamGroup = 1:nStreamGroups
    for thisConfidenceGroup = 1:nConfidenceGroups
        % Open file.
        cd([dataDirectory streamGroups{thisStreamGroup} confidenceGroups{thisConfidenceGroup}]);
        ...
        for thisParticipant = 1:nParticipants
            ...
            allParticipantsGlobal(thisStreamGroup, thisConfidenceGroup, thisParticipant) = allParticipants(thisParticipant);

            % Read in the data.
            thisRead = textscan(fileID,dataFormat,'Delimiter','\t','HeaderLines',1,'MultipleDelimsAsOne',0);

            theseColumns = dataColumns;
        ...
        fileName = [streamGroups{thisStreamGroup} '/CompiledData_TGRSVP_Exp2_' confidenceGroups{thisConfidenceGroup}];
